...
pi0 = -60e3;                    % Sets up preconsolidation stress ~ -130kPa
...
for i =1:NUM_STEPS
   ...
   if Fk < 0
       ...
       while abs(Fk) > tol
           ...
           dFdsig = get_dFdsig(params, sigma, eta);
           dQdsig = get_dQdsig(params, sigma, eta);
           
           pstark = get_pstar(params, sigma, eps_tot, pi);
             
           Hk = get_H(params, sigma, pi, pstark);
           
           fcq = multiply_F_C_Q(params, dFdsig, dQdsig);
           
           % Plastic multiplier
           dlambda = Fk/(fcq + Hk);
         
           % Update
           pi = pi_n - dlambda*h*(pi_n - pstark);
       
           sigma = sigma - dlambda*hookes(dQdsig, params.K, params.G);
           
           p = get_p(sigma);
           q = get_q(sigma);
           
           % Calculate new value of failure function
           Fk = F_func(p, q, pi);
